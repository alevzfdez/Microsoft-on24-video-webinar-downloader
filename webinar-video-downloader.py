...
def webinar_scrapping(url_data):
  ...
  for mediaUrlInfoContent in json.loads(req.content)['mediaUrlInfo']:
    if 'mp4' in mediaUrlInfoContent['url']:
      video_url_1='https://dashod.akamaized.net/media/cv/events/'+mediaUrlInfoContent['sourcefilename'].split('.mp4')[0]+'_mpd/stream.mpd'
      video_url_2='https://dashod.akamaized.net/media/cv/events/'+mediaUrlInfoContent['sourcefilename'].split('.mp4')[0]+'_segments/stream.mpd'
